...
class SpeakerSystem:
    ...
    def get_displacements(self, freqs: np.array) -> tuple:
        ...
        _, x1_t = signal.freqresp(self.ss_models["x1(t)"], w=w)
        _, x2_t = signal.freqresp(self.ss_models["x2(t)"], w=w)
        _, xpr_t = signal.freqresp(self.ss_models["x_pr(t)"], w=w)
        
        disps["Diaphragm, RMS (mm)"] = x1_t * 1e3
        disps["Diaphragm, peak (mm)"] = x1_t * 2**0.5 * 1e3
        disps["Parent body, RMS (mm)"] = x2_t * 1e3
        disps["Parent body, peak (mm)"] = x2_t * 2**0.5 * 1e3
        disps["PR/vent, RMS (mm)"] = xpr_t * 1e3
        disps["PR/vent, peak (mm)"] = xpr_t * 2**0.5 * 1e3
