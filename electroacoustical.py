...
class SpeakerDriver:
    ...
    def __post_init__(self):
        if self.motor is None and self.Rs is not None:
...
class SpeakerSystem:
    ...
    def get_displacements(self, freqs: np.array) -> tuple:
        disps = dict()
        w = 2 * np.pi * np.array(freqs)

        _, x1 = signal.freqresp(self.ss_models["x1(t)"], w=w)
        disps["Diaphragm, RMS (mm)"] = x1 * 1e3
        disps["Diaphragm, peak (mm)"] = x1 * 2**0.5 * 1e3

        if self.parent_body is not None:
            _, x2 = signal.freqresp(self.ss_models["x2(t)"], w=w)
            disps["Parent body, RMS (mm)"] = x2 * 1e3
            disps["Parent body, peak (mm)"] = x2 * 2**0.5 * 1e3

        if self.passive_radiator is not None:
            _, xpr = signal.freqresp(self.ss_models["x_pr(t)"], w=w)
            disps["PR/vent, RMS (mm)"] = xpr * 1e3
            disps["PR/vent, peak (mm)"] = xpr * 2**0.5 * 1e3
        
        return disps

    def get_velocities(self, freqs: np.array) -> tuple:
        velocs = dict()
        w = 2 * np.pi * np.array(freqs)

        _, x1_t = signal.freqresp(self.ss_models["Derivative(x1(t), t)"], w=w)
        velocs["Diaphragm, RMS (m/s)"] = x1_t

        if self.parent_body is not None:
            _, x2_t = signal.freqresp(self.ss_models["Derivative(x2(t), t)"], w=w)
            velocs["Parent body, RMS (m/s)"] = x2_t

        if self.passive_radiator is not None:
            _, xpr_t = signal.freqresp(self.ss_models["Derivative(x_pr(t), t)"], w=w)
            velocs["PR/vent, RMS (m/s)"] = xpr_t
        
        return velocs
    
    def get_Z(self, freqs):
        _, velocs = self.get_velocities(freqs)
        coil_relative_velocity = velocs["Derivative(x1(t), t)"] - velocs["Derivative(x2(t), t)"]
        return self.Rs + self.speaker.Re / (1 - self.speaker.Bl * coil_relative_velocity)

    def get_forces(self, freqs) -> dict:
